function [A_star] = inv_gaussian_transport( z_xi,z_yj, xlength, mean_one, ylength, mean_two)
...
%Closed form solution to transport between two gaussians (sigma_x^-.5(sigma_x^.5*sigma_y*sigma_x^.5)*sigma_x^-.5)
    ...
    A_star =inv(sqrtm(sigma_y))*sqrtm((sqrtm(sigma_y)*sigma_x*sqrtm(sigma_y)))*(inv(sqrtm(sigma_y)));
